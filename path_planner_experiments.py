...
DEFAULT_EXPERIMENT_CSV_PATH = "sweep_strategy_results.csv"
DEFAULT_EXPERIMENT_JSON_PATH = "sweep_strategy_results.json"
DEFAULT_EXPERIMENT_TANKS = (
    ("24ft", "3 Tank examples/24ft.dxf"),
    ("65ft", "3 Tank examples/65ft.dxf"),
    ("150ft", "3 Tank examples/150ft.dxf"),
)
...
def _generate_fixed_circular_sweep_plan(
    ...
    *,
    ...
    """Generate exactly the requested circular rows, unless a row is invalid."""
    ...
    for row_id in range(fixed_circular_rows):
        ...
        if generated is None:
            return CircularSweepPlan(rows, poses, current_radius, "overlap")
...
def build_mission_plan(
    ...
    *,
    ...
    for candidate_factor in circular_spacing_candidates:
        ...
        if fixed_circular_rows is not None and len(mission.circular_rows) != fixed_circular_rows:
            continue
...
def run_sweep_strategy_experiment(
    dxf_path: str | Path,
    *,
    tank_name: str | None = None,
    max_circular_rows: int = 4,
    target_coverage: float = 0.95,
    no_plot: bool = False,
) -> tuple[list[dict[str, Any]], dict[str, Any] | None]:
    """Compare lawnmower-only and fixed circular-row strategies for one tank."""
    path = Path(dxf_path)
    tank_label = tank_name or path.stem
    max_circular_rows = min(max_circular_rows, DEFAULT_MAX_CIRCULAR_ROWS)
    model = import_dxf(path)
    profile_config = RainbowProfileConfig()
    results: list[dict[str, Any]] = []
    best_mission: OuterEdgeSweepMission | None = None
    best_result: dict[str, Any] | None = None

    for row_count in range(max_circular_rows + 1):
        try:
            if row_count == 0:
                mission = build_mission_plan(
                    model,
                    circular_spacing_candidates=None,
                    fixed_circular_rows=0,
                    profile_config=profile_config,
                )
            else:
                mission = build_mission_plan(
            ...
            if len(mission.circular_rows) != row_count:
                raise ValueError(f"only generated {len(mission.circular_rows)} of {row_count} requested rows")
            ...
            result = {
                ...
                "invalid_reason": "",
                "coverage_percent": coverage.covered_percent,
                "hit_95": coverage.covered_percent >= target_coverage * 100.0,
                "scan_count": scan_count,
                "travel_distance_m": travel_distance,
                "avg_travel_per_scan_m": travel_distance / scan_count if scan_count else 0.0,
                "selected_circular_spacing_factor": mission.selected_circular_spacing_factor if row_count else None,
                "circular_stop_reason": mission.circular_stop_reason,
            }
            results.append(result)
            if best_result is None or _experiment_result_is_better(result, best_result, target_coverage):
                best_result = result
            ...
            results.append(
                {
                    ...
                    "avg_travel_per_scan_m": None,
                    "selected_circular_spacing_factor": None,
                    "circular_stop_reason": "",
                }
            )

    _print_experiment_table(tank_label, results, best_result)
    if best_mission is not None and best_result is not None:
        _save_best_experiment_outputs(model, best_mission, best_result, no_plot=no_plot)
    return results, best_result


def _experiment_result_is_better(
    candidate: dict[str, Any],
    current: dict[str, Any],
    target_coverage: float,
) -> bool:
    target_percent = target_coverage * 100.0
    candidate_hit = bool(candidate["coverage_percent"] is not None and candidate["coverage_percent"] >= target_percent)
    current_hit = bool(current["coverage_percent"] is not None and current["coverage_percent"] >= target_percent)
    if candidate_hit != current_hit:
        return candidate_hit
    if candidate_hit and current_hit:
        coverage_gain = candidate["coverage_percent"] - current["coverage_percent"]
        scan_savings = current["scan_count"] - candidate["scan_count"]
        ...
        return (
            ...
            candidate["circular_rows"],
        ) < (
            current["travel_distance_m"],
            current["scan_count"],
            current["circular_rows"],
        )

    coverage_delta = candidate["coverage_percent"] - current["coverage_percent"]
...
def _print_experiment_table(
    ...
    print(f"\nSweep strategy experiment: {tank_label}")
    print("tank | circular_rows | coverage_% | hit_95 | scans | travel_m | avg_travel_per_scan | selected_circular_spacing")
    for result in results:
        if not result["valid"]:
            print(
                f"{result['tank']} | {result['circular_rows']:<13} | invalid    | no     | "
                f"n/a   | n/a      | n/a                 | {result['invalid_reason']}"
            )
            continue
        selected = "  <-- best" if best_result is result else ""
        spacing = result["selected_circular_spacing_factor"]
        spacing_text = "n/a" if spacing is None else f"{spacing:.2f}"
        print(
            f"{result['tank']} | {result['circular_rows']:<13} | "
            f"{result['coverage_percent']:<10.1f} | "
            f"{'yes' if result['hit_95'] else 'no':<6} | "
            f"{result['scan_count']:<5} | "
            f"{result['travel_distance_m']:<8.1f} | "
            f"{result['avg_travel_per_scan_m']:<19.2f} | "
            f"{spacing_text}{selected}"
        )
    if best_result is not None:
        print(
            f"Best strategy for {tank_label}: {best_result['circular_rows']} circular rows, "
            f"{best_result['coverage_percent']:.1f}% coverage, "
            f"{best_result['scan_count']} scans, "
            f"{best_result['travel_distance_m']:.1f} m travel"
        )


def _save_best_experiment_outputs(
    model: GeometryModel,
    ...
    *,
    ...
    tank_label = str(best_result["tank"])
    safe_label = tank_label.replace(" ", "_")
    json_path = Path(f"best_{safe_label}_mission_plan.json")
    preview_path = Path(f"best_{safe_label}_mission_preview.png")
    save_mission_json(mission, json_path)
    title = (
        f"{tank_label}: {best_result['circular_rows']} circular rows, "
        f"{best_result['coverage_percent']:.1f}% coverage, "
        f"{best_result['scan_count']} scans, "
        f"{best_result['travel_distance_m']:.1f} m travel, "
        f"spacing {best_result['selected_circular_spacing_factor'] if best_result['selected_circular_spacing_factor'] is not None else 'n/a'}"
    )
...
def save_experiment_results(
    results: list[dict[str, Any]],
    *,
    csv_path: str | Path = DEFAULT_EXPERIMENT_CSV_PATH,
    json_path: str | Path = DEFAULT_EXPERIMENT_JSON_PATH,
) -> tuple[Path, Path]:
    ...
    fieldnames = [
        ...
        "scan_count",
        "travel_distance_m",
        "avg_travel_per_scan_m",
        "selected_circular_spacing_factor",
        "circular_stop_reason",
    ]
...
def plot_outer_edge_sweep(
    ...
    *,
    ...
    elif mission.selected_circular_spacing_factor is None:
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("dxf_path", nargs="?", help="Path to the tank floor DXF file.")
    parser.add_argument("--experiment", action="store_true", help="Compare circular row counts for one tank DXF.")
    parser.add_argument("--experiment-all", action="store_true", help="Run sweep strategy experiments for all example tanks.")
    parser.add_argument(
        "--max-circular-rows",
        type=int,
        default=DEFAULT_MAX_CIRCULAR_ROWS,
        help=f"Maximum circular row count to test in experiment mode. Capped at {DEFAULT_MAX_CIRCULAR_ROWS}.",
    )
    parser.add_argument(
        "--target-coverage",
        type=float,
        default=0.95,
        help="Coverage target used for experiment best-strategy selection. Default: 0.95.",
    )
    parser.add_argument(
        "--outer-edge-offset",
        "--edge-offset",
...
def main() -> int:
    ...
    if args.experiment or args.experiment_all:
        try:
            if args.experiment_all:
                experiment_targets = [(name, Path(path)) for name, path in DEFAULT_EXPERIMENT_TANKS]
            else:
                if args.dxf_path is None:
                    raise ValueError("A DXF path is required with --experiment.")
                experiment_targets = [(Path(args.dxf_path).stem, Path(args.dxf_path))]

            all_results: list[dict[str, Any]] = []
            for tank_name, dxf_path in experiment_targets:
                results, _best_result = run_sweep_strategy_experiment(
                    dxf_path,
                    tank_name=tank_name,
                    max_circular_rows=args.max_circular_rows,
                    target_coverage=args.target_coverage,
                    no_plot=args.no_plot,
                )
                all_results.extend(results)
            csv_path, json_path = save_experiment_results(all_results)
            print(f"\nExperiment CSV output: {csv_path}")
            print(f"Experiment JSON output: {json_path}")
        except (DxfImportError, ValueError) as exc:
            print(f"Sweep strategy experiment failed: {exc}")
            return 1
        return 0

    if args.dxf_path is None:
        print("Planning failed: a DXF path is required unless --experiment-all is used.")
        return 1
